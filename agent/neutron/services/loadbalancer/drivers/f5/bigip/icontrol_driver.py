...
class iControlDriver(object):
    ...
    def _assure_update_member(self, bigip, on_last_bigip,
        ...
        if network:
            ...
            if network['provider:network_type'] == 'vxlan' \
                ...
                self.bigip_l2_manager.add_vxlan_fdbs(
                    bigip, net_folder, network,
                    member['vxlan_vteps'],
                    ip_address,
                    member['port']['mac_address'])

            if network['provider:network_type'] == 'gre' \
                    and 'gre_vteps' in member:
                self.bigip_l2_manager.add_gre_fdbs(
                    bigip, net_folder, network,
                    member['gre_vteps'],
                    ip_address,
                    member['port']['mac_address'])
    ...
    def _assure_delete_member(self, bigip, on_last_bigip,
        ...
        if network:
            if member['port']:
                ...
                if network['provider:network_type'] == 'vxlan' \
                        and 'vxlan_vteps' in member:
                    self.bigip_l2_manager.delete_vxlan_fdbs(
                        bigip, net_folder, network,
                        member['vxlan_vteps'],
                        ip_address,
                        member['port']['mac_address'])
                ...
                if network['provider:network_type'] == 'gre' \
                        and 'gre_vteps' in member:
                    self.bigip_l2_manager.delete_gre_fdbs(
                        bigip, net_folder, network,
                        member['gre_vteps'],
                        ip_address,
                        member['port']['mac_address'])
    ...
    def _delete_device_vip(self, service, bigip, on_last_bigip):
        ...
        if network:
            ...
            if network['provider:network_type'] == 'vxlan' \
                ...
                self.bigip_l2_manager.delete_vxlan_fdbs(
                    bigip, net_folder, network,
                    vip['vxlan_vteps'], None, None)
            if network['provider:network_type'] == 'gre' \
                    and 'gre_vteps' in vip:
                self.bigip_l2_manager.delete_gre_fdbs(
                    bigip, net_folder, network,
                    vip['gre_vteps'], None, None)
    ...
    def _create_device_vip(self, service, bigip, on_last_bigip,
        ...
        if virtual_type == 'standard':
            ...
            if bigip_vs.create_fastl4(name=vs_name,
                                      ip_address=ip_address,
                                      mask='255.255.255.255',
                                      port=int(vip['protocol_port']),
                                      protocol=vip['protocol'],
        ...
        if 'session_persistence' in vip and vip['session_persistence']:
            ...
            if persistence_type == 'SOURCE_IP':
                ...
                if 'cookie_name' in vip['session_persistence']:
                    ...
                    create_r = self._create_app_cookie_persist_rule
                    rule_definition = create_r(cookie_name)
    ...
    def _assure_delete_networks(self, service, bigip, ctxs):
        ...
        for bigip in bigips:
            on_last_bigip = (bigip is bigips[-1])
            ctx = ctxs[bigip.device_name]
            self._assure_device_delete_networks(service, bigip,
                                                ctx, on_last_bigip)
    ...
    def _assure_network(self, network, bigip, on_first_bigip):
        ...
        for bigip in bigips:
            if network['id'] in bigip.assured_networks:
                continue
            self.bigip_l2_manager.assure_device_network(network, bigip)
    ...
    def _assure_snats_ha(self, bigip, subnetinfo,
        ...
        for i in range(self.conf.f5_snat_addresses_per_subnet):
            ...
            for set_bigip in bigips:
                if subnet['id'] in set_bigip.assured_snat_subnets:
                    continue
                set_bigip.snat.create(name=index_snat_name,
                                      ip_address=ip_address,
        ...
        if not self.connected:
            try:
                ...
                LOG.info(_('Opening iControl connections to %s @ %s' %
                           (self.username, self.hostnames[0])))
                # connect to inital device:
                first_bigip = f5_bigip.BigIP(
                    self.hostnames[0],
                    self.username,
                    self.password,
                    f5const.CONNECTION_TIMEOUT,
                ...
                if not cluster_name and self.conf.f5_ha_type != 'standalone':
                    raise f5ex.BigIPClusterInvalidHA(
                        ...
                        'device group found.' % self.conf.f5_ha_type)
                ...
                for host in self.hostnames[1:]:
                    ...
                    if major_version < f5const.MIN_TMOS_MAJOR_VERSION:
                        raise f5ex.MajorVersionValidateFailed(
                            'device %s must be at least TMOS %s.%s'
                            % (host, f5const.MIN_TMOS_MAJOR_VERSION,
                               f5const.MIN_TMOS_MINOR_VERSION))
